...
Mealie is a self-hosted recipe manager, meal planner, and shopping list application with a FastAPI backend (Python 3.12) and a React frontend (React 19 + TypeScript + Vite). It uses SQLAlchemy ORM with support for SQLite and PostgreSQL databases.

**Development vs Production:**
- **Development:** Frontend (port 4173) and backend (port 9000) run as separate processes
- **Production:** Frontend is statically built from `frontend-react/` and served by nginx in Docker
...
- TypeScript types auto-generated from Pydantic schemas - **never manually edit** `frontend-react/src/lib/api/types/`
...
**API Client Pattern:**
- Shared API client utilities live in `frontend-react/src/lib/api/`
- Types imported from auto-generated `frontend-react/src/lib/api/types/` (DO NOT EDIT MANUALLY)
- Feature-specific API helpers live alongside features (for example `frontend-react/src/features/**/api.ts`)
- Auth state and session helpers live under `frontend-react/src/features/auth/`

**State Management:**
- Use React hooks, React Query, and feature-local state
- Auth state is managed via `frontend-react/src/features/auth/session.ts` and related hooks
- Prefer shared hooks/utilities over duplicating inline state logic
...
task ui                 # Start React frontend (port 4173)
...
1. **Code generation is source of truth:** After Pydantic schema changes, run `task dev:generate` to update:
   - TypeScript types (`frontend-react/src/lib/api/types/`)
...
**Generated Files:**
- Verify `frontend-react/src/lib/api/types/` files weren't manually edited (they're auto-generated)
...
- **Don't manually edit generated files:** `frontend-react/src/lib/api/types/`, schema `__init__.py` files
