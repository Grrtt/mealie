...
public interface IMealPlanRuleService
{
    ...
    Task<MealPlanRuleResponse> CreateAsync(Guid groupId, Guid householdId, CreateMealPlanRuleRequest request, CancellationToken ct = default);
    Task<MealPlanRuleResponse?> UpdateAsync(Guid groupId, Guid id, UpdateMealPlanRuleRequest request, CancellationToken ct = default);
