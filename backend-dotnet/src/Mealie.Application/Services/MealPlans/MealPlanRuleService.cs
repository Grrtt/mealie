...
public class MealPlanRuleService(ApplicationDbContext db) : IMealPlanRuleService
{
    ...
    public async Task<IList<MealPlanRuleResponse>> GetAllAsync(Guid groupId, Guid householdId, CancellationToken ct = default)
    {
    ...
    public async Task<MealPlanRuleResponse> CreateAsync(Guid groupId, Guid householdId, CreateMealPlanRuleRequest request, CancellationToken ct = default)
    {
    ...
    public async Task<MealPlanRuleResponse?> UpdateAsync(Guid groupId, Guid id, UpdateMealPlanRuleRequest request, CancellationToken ct = default)
    {
        var rule = await WithNav(db.MealPlanRules.IgnoreQueryFilters())
            .FirstOrDefaultAsync(r => r.GroupId == groupId && r.Id == id, ct);
    ...
    private async Task AttachNavigations(MealPlanRule rule, IList<Guid> tagIds, IList<Guid> categoryIds, IList<Guid> householdIds, CancellationToken ct)
    {
        if (tagIds.Count > 0)
        {
    ...
    private static MealPlanRuleResponse MapToResponse(MealPlanRule r) => new()
    {
        Id = r.Id,
        GroupId = r.GroupId,
