...
public static class RecipeImageProcessor
{
    ...
    {
        var scale = (float)size / Math.Min(src.Width, src.Height);
        var scaledW = (int)Math.Ceiling(src.Width * scale);
        var scaledH = (int)Math.Ceiling(src.Height * scale);
        ...
        var x = (scaledW - size) / 2;
        var y = (scaledH - size) / 2;
    ...
    private static (int w, int h) FitSize(int srcW, int srcH, int maxW, int maxH)
    {
        ...
        var ratio = Math.Min((float)maxW / srcW, (float)maxH / srcH);
