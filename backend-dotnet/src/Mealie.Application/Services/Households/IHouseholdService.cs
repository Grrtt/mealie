...
public interface IHouseholdService
{
    ...
    Task<HouseholdResponse?> UpdateHouseholdAsync(Guid householdId, UpdateHouseholdRequest request,
        CancellationToken ct = default);

    Task<HouseholdResponse> CreateHouseholdAsync(Guid groupId, CreateHouseholdRequest request,
        CancellationToken ct = default);
    ...
    Task<HouseholdPreferencesResponse?> UpdateHouseholdPreferencesAsync(Guid householdId,
        UpdateHouseholdPreferencesRequest request, CancellationToken ct = default);

    Task<InviteTokenResponse> CreateHouseholdInviteTokenAsync(Guid groupId, Guid householdId,
        CreateInviteTokenRequest request, CancellationToken ct = default);

    Task<bool> UpdateMemberPermissionsAsync(Guid householdId, Guid userId, bool admin, bool canOrganize, bool canInvite,
        CancellationToken ct = default);
}
