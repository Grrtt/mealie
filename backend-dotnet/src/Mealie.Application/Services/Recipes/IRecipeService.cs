...
public interface IRecipeService
{
    ...
    Task<PaginatedResponse<RecipeSummaryResponse>> GetPaginatedAsync(Guid householdId, PaginationParams pagination,
        RecipeFilter? filter = null, CancellationToken ct = default);
    ...
    Task<RecipeDetailResponse> CreateAsync(Guid groupId, Guid householdId, Guid userId, CreateRecipeRequest request,
        CancellationToken ct = default);

    Task<RecipeDetailResponse?> UpdateAsync(Guid groupId, string slug, UpdateRecipeRequest request,
        CancellationToken ct = default);

    Task<bool> DeleteAsync(Guid groupId, string slug, CancellationToken ct = default);

    Task<RecipeDetailResponse?> DuplicateAsync(Guid groupId, Guid householdId, Guid userId, string slug,
        CancellationToken ct = default);
    ...
    Task BulkCategorizeAsync(IList<string> slugs, IList<string> categoryNames, Guid groupId,
        CancellationToken ct = default);

    Task<RecipeSuggestionsResponse> GetSuggestionsAsync(Guid householdId, Guid groupId, int limit, string? queryFilter,
        int maxMissingFoods, int maxMissingTools, bool includeFoodsOnHand, bool includeToolsOnHand, IList<Guid> foodIds,
        IList<Guid> toolIds, CancellationToken ct = default);
