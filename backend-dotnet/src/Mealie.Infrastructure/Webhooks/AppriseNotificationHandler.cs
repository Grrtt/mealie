      ...
      INotificationHandler<UserSignedUpEvent>
{
    public Task Handle(RecipeCreatedEvent notification, CancellationToken ct)
        => DispatchAsync(notification.HouseholdId, "recipe_created",
    ...
    public Task Handle(RecipeUpdatedEvent notification, CancellationToken ct)
        => DispatchAsync(notification.HouseholdId, "recipe_updated",
    ...
    public Task Handle(RecipeDeletedEvent notification, CancellationToken ct)
        => DispatchAsync(notification.HouseholdId, "recipe_deleted",
    ...
    public Task Handle(MealPlanEntryCreatedEvent notification, CancellationToken ct)
        => DispatchAsync(notification.HouseholdId, "mealplan_entry_created",
    ...
    public Task Handle(MealPlanEntryUpdatedEvent notification, CancellationToken ct)
        => DispatchAsync(notification.HouseholdId, "mealplan_entry_updated",
    ...
    public Task Handle(MealPlanEntryDeletedEvent notification, CancellationToken ct)
        => DispatchAsync(notification.HouseholdId, "mealplan_entry_deleted",
    ...
    public Task Handle(ShoppingListCreatedEvent notification, CancellationToken ct)
        => DispatchAsync(notification.HouseholdId, "shopping_list_created",
    ...
    public Task Handle(ShoppingListUpdatedEvent notification, CancellationToken ct)
        => DispatchAsync(notification.HouseholdId, "shopping_list_updated",
    ...
    public Task Handle(ShoppingListDeletedEvent notification, CancellationToken ct)
        => DispatchAsync(notification.HouseholdId, "shopping_list_deleted",
    ...
    public Task Handle(UserSignedUpEvent notification, CancellationToken ct)
        => DispatchAsync(notification.HouseholdId, "user_signup",
            new { event_type = "user_signup", user_id = notification.UserId, username = notification.Username, email = notification.Email }, ct,
            o => o.UserSignup);
    ...
    private async Task DispatchAsync(
        ...
        Func<Domain.Entities.Settings.EventNotifierOptions, bool> optionSelector)
    {
        ...
        foreach (var notifier in notifiers)
        {
            ...
            {
                logger.LogDebug("Skipping non-HTTP Apprise URL for event {EventType}: {Url}", eventType, notifier.ApprisUrl);
                continue;
            }
            ...
            try
            {
                ...
                if (!response.IsSuccessStatusCode)
                    logger.LogWarning("Apprise delivery failed for {EventType}. Url={Url} Status={Status}", eventType, notifier.ApprisUrl, (int)response.StatusCode);
            }
            catch (Exception ex)
            {
                logger.LogWarning(ex, "Apprise delivery exception for {EventType}. Url={Url}", eventType, notifier.ApprisUrl);
            }
        }
    }
}
