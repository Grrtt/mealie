    ...
    IOptions<AppSettings> settings) : ControllerBase
{
    ...
    public async Task<IActionResult> ChangePassword([FromBody] ChangePasswordRequest request,
        CancellationToken ct = default)
    {
        return await ChangePasswordSelf(request, ct);
    }
    ...
    {
        return Ok(await executor.ExecuteAsync(new GetApiKeysQuery(tenantContext.UserId), ct));
    }
    ...
    public async Task<ActionResult<ApiKeyResponse>> CreateApiToken([FromBody] CreateApiKeyRequest request,
        CancellationToken ct = default)
    {
        return Ok(await executor.ExecuteAsync(new CreateApiKeyCommand(tenantContext.UserId, request.Name), ct));
    }
    ...
    {
        return await DeleteApiToken(tokenId, ct);
    }
    ...
    {
        return Ok(await executor.ExecuteAsync(new GetFavoriteRecipesQuery(userId), ct));
    }
    ...
    {
        return Ok(await executor.ExecuteAsync(new GetUserRatingsQuery(tenantContext.UserId), ct));
    }
    ...
    {
        return Ok(await executor.ExecuteAsync(new GetUserRatingsQuery(userId), ct));
    }
