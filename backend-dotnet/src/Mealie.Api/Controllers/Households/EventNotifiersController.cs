...
public class EventNotifiersController(IEventNotifierService notifierService, ITenantContext tenantContext)
    : MealieControllerBase(tenantContext)
{
    [HttpGet]
    public async Task<IActionResult> GetAll()
    {
        return Ok(await notifierService.GetAllAsync(CurrentHouseholdId));
    }

    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateEventNotifierRequest request)
    {
        return Ok(await notifierService.CreateAsync(CurrentGroupId, CurrentHouseholdId, request));
    }
