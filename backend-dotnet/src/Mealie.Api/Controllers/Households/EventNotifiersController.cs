    ...
    : MealieControllerBase(tenantContext)
{
    ...
    public async Task<IActionResult> GetAll(CancellationToken ct = default)
        => Ok(await executor.ExecuteAsync(new GetEventNotifiersQuery(CurrentHouseholdId), ct));

    [HttpPost]
    public async Task<IActionResult> Create([FromBody] CreateEventNotifierRequest request, CancellationToken ct = default)
        => Ok(await executor.ExecuteAsync(new CreateEventNotifierCommand(CurrentGroupId, CurrentHouseholdId, request), ct));

    [HttpPut("{itemId:guid}")]
    public async Task<IActionResult> Update(Guid itemId, [FromBody] CreateEventNotifierRequest request, CancellationToken ct = default)
    {
