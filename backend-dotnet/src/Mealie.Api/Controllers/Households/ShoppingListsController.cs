...
public class ShoppingListsController(IShoppingListService shoppingListService, ITenantContext tenantContext)
    : MealieControllerBase(tenantContext)
{
    ...
    {
        return Ok(await shoppingListService.GetShoppingListsAsync(CurrentHouseholdId, pagination, ct));
    }
    ...
    [HttpPatch("{id:guid}")]
    public async Task<ActionResult<ShoppingListResponse>> PatchShoppingList(Guid id,
        [FromBody] UpdateShoppingListRequest request, CancellationToken ct)
    {
        var list = await shoppingListService.UpdateAsync(CurrentHouseholdId, id, request, ct);
        if (list is null)
