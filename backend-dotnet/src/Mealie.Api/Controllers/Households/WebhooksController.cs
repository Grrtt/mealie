    ...
    : MealieControllerBase(tenantContext)
{
    ...
    {
        return Ok(await executor.ExecuteAsync(new GetWebhooksQuery(CurrentHouseholdId), ct));
    }
    ...
    {
        return Ok(
            await executor.ExecuteAsync(new CreateWebhookCommand(CurrentGroupId, CurrentHouseholdId, request), ct));
    }
