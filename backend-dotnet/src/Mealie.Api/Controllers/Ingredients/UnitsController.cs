...
[ApiController]
[Route("api/units")]
public class UnitsController(IUnitService unitService, ITenantContext tenantContext) : MealieControllerBase(tenantContext)
{
    [HttpGet]
    public async Task<ActionResult<PaginatedResponse<UnitResponse>>> GetUnits(
        ...
        => Ok(await unitService.GetUnitsAsync(CurrentGroupId, pagination, ct));
    ...
    [HttpPut("{id:guid}")]
    public async Task<ActionResult<UnitResponse>> UpdateUnit(Guid id, [FromBody] UpdateUnitRequest request, CancellationToken ct)
    {
