...
public class GroupLabelsController(IGroupLabelService groupLabelService, ITenantContext tenantContext) : MealieControllerBase(tenantContext)
{
    [HttpGet]
    public async Task<ActionResult<PaginatedResponse<GroupLabelResponse>>> GetLabels(
        ...
        => Ok(await groupLabelService.GetLabelsAsync(CurrentGroupId, pagination, ct));
    ...
    public async Task<ActionResult<GroupLabelResponse>> GetLabel(Guid id, CancellationToken ct)
    {
        ...
        if (label is null) return NotFoundOrForbidden();
        return Ok(label);
    }

    [HttpPost]
    public async Task<ActionResult<GroupLabelResponse>> CreateLabel([FromBody] CreateGroupLabelRequest request, CancellationToken ct)
    {
    ...
    [HttpPut("{id:guid}")]
    public async Task<ActionResult<GroupLabelResponse>> UpdateLabel(Guid id, [FromBody] UpdateGroupLabelRequest request, CancellationToken ct)
    {
    ...
    [HttpPatch("{id:guid}")]
    public async Task<ActionResult<GroupLabelResponse>> PatchLabel(Guid id, [FromBody] UpdateGroupLabelRequest request, CancellationToken ct)
    {
        var label = await groupLabelService.UpdateAsync(CurrentGroupId, id, request, ct);
        if (label is null) return NotFoundOrForbidden();
        return Ok(label);
