    ...
    bool dryRun)
{
    ...
    public async Task<int> RunAsync()
    {
        ...
        await ExecuteTierAsync("Tier2 (Organizers)", () => new Tier2Reader(sourceConn).ReadAllAsync(), targetConn, report);
        await ExecuteTierAsync("Tier3 (Recipes)", () => new Tier3Reader(sourceConn).ReadAllAsync(), targetConn, report);
        await ExecuteTierAsync("Tier4 (Junction)", () => new Tier4Reader(sourceConn).ReadAllAsync(), targetConn, report);
    ...
    private async Task ExecuteTierAsync(string tierName, Func<Task<MigrationData>> readFn, DbConnection targetConn, MigrationReport report)
    {
    ...
    {
        await conn.ExecuteAsync("INSERT INTO __mealie_migration_log (migrated_at, notes) VALUES (CURRENT_TIMESTAMP, 'Completed by Mealie.Migration CLI')");
    }
}
