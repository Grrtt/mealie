...
public class TargetWriter(DbConnection conn, ILogger log)
{
    public async Task WriteAsync(MigrationData data, MigrationReport report)
    {
        await WriteEntitiesAsync("groups", data.Groups, report, async (row) =>
        {
            await conn.ExecuteAsync("""
                ...
                """, new { id = (string)row.id, name = (string)row.name, slug = (string?)row.slug, created_at = row.created_at, update_at = row.update_at });
        });
        ...
        await WriteEntitiesAsync("households", data.Households, report, async (row) =>
        {
            await conn.ExecuteAsync("""
                ...
                ON CONFLICT (id) DO NOTHING
                """, new { id = (string)row.id, name = (string)row.name, slug = (string?)row.slug, group_id = (string)row.group_id, created_at = row.created_at, update_at = row.update_at });
        });
    ...
    private async Task WriteEntitiesAsync(string entityName, List<dynamic> rows, MigrationReport report, Func<dynamic, Task> insertFn)
    {
    ...
    {
        try { return (string)row.id; } catch { return "unknown"; }
    }
}
